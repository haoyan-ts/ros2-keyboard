...
class KeyboardToTextNode(Node):
    def __init__(self):
        # Initialize ROS node
        super().__init__("keyboard_to_text_node")
    ...
    def keydown_callback(self, msg):
        self.get_logger().info(f'keydown! {msg.code}')
        # for part in self.axes + self.buttons:
        #     part.down(msg.code)

    def keyup_callback(self, msg):
        self.get_logger().info(f'keyup! {msg.code}')
